...
def is_free_function(py_object, full_name, index):
  ...
  return not tf_inspect.isclass(index[parent_name])
...
class _Errors(object):
  ...
  def __eq__(self, other):
    return self._errors == other._errors if isinstance(other, _Errors) else False
...
def documentation_path(full_name, is_fragment=False):
  ...
  result = f'{os.path.join(*parts)}.md'
...
class ReferenceResolver(object):
  ...
  def to_json_file(self, filepath):
    ...
    json_dict = {
        ...
        if key not in ('_doc_index', '_all_names', '_errors',
                       'current_doc_full_name')
    }
  ...
  def reference_to_url(self, ref_full_name, relative_path_to_root):
    ...
    if master_name not in self._all_names:
      raise TFDocsError(f'Cannot make link to "{master_name}": Not in index.')
  ...
  def _one_ref(self, match, relative_path_to_root):
    ...
    if string.startswith('$'):  # Doc reference
      ...
      is_python = any(
          string == py_module_name or string.startswith(f'{py_module_name}.')
    ...
    # Error!
    raise TFDocsError(f'Did not understand "{match.group(0)}"', 'BROKEN_LINK')
  ...
  def _doc_link(self, string, link_text, manual_link_text,
    ...
    if string in self._doc_index:
      ...
      link_text = self._link_text_to_html(link_text)
      return f'<a href="{url}{hash_tag}">{link_text}</a>'
  ...
  def _doc_missing(self, string, unused_hash_tag, unused_link_text,
    ...
    raise TFDocsError(f'Unknown Document "{string}"')
...
def _remove_first_line_indent(string):
  indent = len(re.match(r'^\s*', string)[0])
...
def _generate_signature(func, reverse_index):
  ...
  args_list = list(argspec.args[first_arg:first_arg_with_default])
    ...
    for arg, default, ast_default in zip(
      ...
      if id(default) in reverse_index:
        ...
        if default_text != repr(default):
          ...
          if match := re.match(full_name_re, default_text):
            ...
            for internal_name, public_name in six.iteritems(internal_names):
              if match[0].startswith(internal_name):
  ...
  # Add *args and *kwargs.
  if argspec.varargs:
    args_list.append(f'*{argspec.varargs}')
  if argspec.varkw:
    args_list.append(f'**{argspec.varkw}')
...
def _get_guides_markdown(duplicate_names, guide_index, relative_path):
  ...
  links = sorted({guide_ref.make_md_link(prefix) for guide_ref in all_guides})
...
def _get_defining_class(py_class, name):
  return next(
      (cls for cls in tf_inspect.getmro(py_class) if name in cls.__dict__),
      None,
  )
...
class _ClassPageInfo(object):
  ...
  def properties(self):
    ...
    if self.namedtuplefields:
      props.extend(props_dict.pop(field) for field in self.namedtuplefields)
...
class _ModulePageInfo(object):
  ...
  def collect_docs_for_module(self, parser_config):
    ...
    for name in member_names:
      ...
      member_full_name = f'{self.full_name}.{name}' if self.full_name else name
...
def _get_defined_in(py_object, parser_config):
  ...
  if 'genfiles' in path or 'tools/api/generator' in path:
    ...
    return _ProtoFile(f'{path[:-7]}.proto', parser_config)
  else:
...
class _Metadata(object):
  ...
  def build_html(self):
    ...
    parts = [
        f'<div itemscope itemtype="{schema}">',
        f'<meta itemprop="name" content="{self.name}" />',
        f'<meta itemprop="path" content="{self.version}" />',
    ]

    parts.extend(f'<meta itemprop="property" content="{item}"/>'
